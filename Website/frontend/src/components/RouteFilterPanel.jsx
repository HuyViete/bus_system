...
const RouteFilterPanel = ({ isOpen, onClose, selectedRouteIds, onSelectionChange }) => {
    ...
    return (
        <div className='fixed top-4 left-16 z-50 w-80 max-h-[calc(100vh-2rem)] bg-white/80 backdrop-blur-xl border border-white/40 shadow-2xl rounded-2xl flex flex-col overflow-hidden transition-all duration-300'>
            ...
            <div className='p-3 border-b border-gray-200/50 bg-gray-50/30'>
                <input 
                    type="text" 
                    placeholder="Search route (e.g. 01, Ga Ba Son)..." 
                    value={searchQuery}
                        ...
                        {filteredRoutes.map(route => {
                            ...
                            return (
                                <button 
                                    ...
                                    className={`w-full text-left p-2.5 rounded-xl flex items-center gap-3 transition-all duration-200 border border-transparent
